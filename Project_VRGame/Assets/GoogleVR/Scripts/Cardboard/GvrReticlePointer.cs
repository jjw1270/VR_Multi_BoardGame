...
public class GvrReticlePointer : GvrBasePointer
{
    ...
    private bool SetPointerTarget(RaycastResult target, bool interactive)
    {   
        //Debug.Log(target);
        ...
        Vector3 targetLocalPosition = PointerTransform.InverseTransformPoint(target.worldPosition);
        ...
        if (interactive)
        {
            ...
            if(Input.GetMouseButtonDown(0)){
                Debug.Log("클릭");
                if(target.gameObject.CompareTag("EnterRoom")){
                    //방 입장
                    if(GameObject.Find("LobyManager")!=null){
                        LobyManager LM = GameObject.Find("LobyManager").GetComponent<LobyManager>();
                        LM.EnterRoom(target.gameObject);
                    }
                }
                else if(target.gameObject.CompareTag("Exit")){
                     Application.Quit();
                }
            }
